...
public interface EventMapper {
    ...
    @Insert("INSERT INTO security_event_attribute (event_id, attribute_key, attribute_value) VALUES (#{eventId}, #{attributeKey}, #{attributeValue})")
    int insertAttribute(@Param("eventId") String eventId, @Param("attributeKey") String attributeKey,
                        @Param("attributeValue") String attributeValue);
    @Insert("INSERT INTO security_event_input_issue (event_id, issue_index, rule_id, fact_name, issue_code, source_type) VALUES (#{eventId}, #{issueIndex}, #{ruleId}, #{factName}, #{issueCode}, #{sourceType})")
    int insertInputIssue(SecurityEventInputIssuePo issue);
    @Insert("INSERT INTO security_event_fact (event_id, fact_key, value_type, value_text, source_type) VALUES (#{eventId}, #{factKey}, #{valueType}, #{valueText}, #{sourceType})")
    ...
    @Select("SELECT event_id AS eventId, fact_key AS factKey, value_type AS valueType, value_text AS valueText, source_type AS sourceType FROM security_event_fact WHERE event_id = #{eventId} ORDER BY fact_key")
