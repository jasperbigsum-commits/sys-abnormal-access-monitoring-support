...
public interface ManagementQueryMapper {
    ...
    @Select("SELECT a.alert_id AS id, a.status, a.version,(SELECT d.assignee_id FROM alert_disposition d WHERE d.alert_id=a.alert_id AND d.assignee_id IS NOT NULL ORDER BY d.created_at DESC,d.disposition_id DESC LIMIT 1) AS assigneeId FROM security_alert a WHERE EXISTS (SELECT 1 FROM alert_event_link l JOIN security_event e ON e.event_id=l.event_id WHERE l.alert_id=a.alert_id AND e.system_id=#{scope}) ORDER BY a.last_seen DESC, a.alert_id ASC LIMIT #{limit} OFFSET #{offset}")
    ...
    @Select("SELECT a.alert_id AS id, a.status, a.version,(SELECT d.assignee_id FROM alert_disposition d WHERE d.alert_id=a.alert_id AND d.assignee_id IS NOT NULL ORDER BY d.created_at DESC,d.disposition_id DESC LIMIT 1) AS assigneeId FROM security_alert a WHERE a.alert_id=#{id} AND EXISTS (SELECT 1 FROM alert_event_link l JOIN security_event e ON e.event_id=l.event_id WHERE l.alert_id=a.alert_id AND e.system_id=#{scope})")
    ManagementRowPo alert(@Param("scope") String scope, @Param("id") String id);
    @Select({"<script>SELECT d.disposition_id AS id,d.alert_id AS alertId,d.operator_id AS operatorId,d.assignee_id AS assigneeId,d.comment_text AS reason,d.expected_version AS expectedVersion,d.created_at AS createdAt FROM alert_disposition d",
        "WHERE d.alert_id=#{id} AND d.assignee_id IS NOT NULL AND EXISTS (SELECT 1 FROM alert_event_link l JOIN security_event e ON e.event_id=l.event_id WHERE l.alert_id=d.alert_id AND e.system_id=#{scope})",
    ...
    @Select("SELECT COUNT(*) FROM alert_disposition d WHERE d.alert_id=#{id} AND d.assignee_id IS NOT NULL AND EXISTS (SELECT 1 FROM alert_event_link l JOIN security_event e ON e.event_id=l.event_id WHERE l.alert_id=d.alert_id AND e.system_id=#{scope})")
    ...
    @Update("UPDATE security_alert SET status='IN_PROGRESS', version=version+1 WHERE alert_id=#{id} AND version=#{version} AND status IN ('NEW','ACKNOWLEDGED') AND EXISTS (SELECT 1 FROM alert_event_link l JOIN security_event e ON e.event_id=l.event_id WHERE l.alert_id=security_alert.alert_id AND e.system_id=#{scope})")
    int assignAlert(@Param("scope") String scope, @Param("id") String id, @Param("version") long version);
    @Insert("INSERT INTO alert_disposition(disposition_id,alert_id,disposition_type,operator_id,assignee_id,expected_version,comment_text,evidence_summary,created_at) VALUES(#{dispositionId},#{alertId},'IN_PROGRESS',#{actorId},#{assigneeId},#{version},#{reason},NULL,CURRENT_TIMESTAMP)")
    ...
    @Select("SELECT d.disposition_id AS id,d.alert_id AS alertId,d.operator_id AS operatorId,d.assignee_id AS assigneeId,d.comment_text AS reason,d.expected_version AS expectedVersion,d.created_at AS createdAt FROM alert_disposition d WHERE d.disposition_id=#{dispositionId} AND d.alert_id=#{id} AND EXISTS (SELECT 1 FROM alert_event_link l JOIN security_event e ON e.event_id=l.event_id WHERE l.alert_id=d.alert_id AND e.system_id=#{scope})")
    AlertAssignmentPo alertAssignment(@Param("scope") String scope, @Param("id") String id,
        @Param("dispositionId") String dispositionId);

    @Select("SELECT r.rule_id AS id, r.rule_mode AS status, r.rule_version AS version, r.rule_threshold AS threshold FROM security_rule r WHERE r.system_id=#{scope} AND r.rule_version=(SELECT MAX(v.rule_version) FROM security_rule v WHERE v.system_id=r.system_id AND v.rule_id=r.rule_id) ORDER BY r.rule_id LIMIT #{limit} OFFSET #{offset}")
    List<ManagementRowPo> rules(@Param("scope") String scope, @Param("limit") int limit, @Param("offset") long offset);
    @Select("SELECT COUNT(DISTINCT rule_id) FROM security_rule WHERE system_id=#{scope}") long countRules(@Param("scope") String scope);
    @Select("SELECT rule_id AS id, rule_mode AS status, rule_version AS version, rule_threshold AS threshold FROM security_rule WHERE system_id=#{scope} AND rule_id=#{id} ORDER BY rule_version DESC LIMIT 1")
    ManagementRowPo rule(@Param("scope") String scope, @Param("id") String id);
    @Insert("INSERT INTO security_rule(system_id,rule_id,rule_version,rule_name,rule_definition,risk_level,rule_mode,rule_threshold,enabled,created_at,created_by,change_reason,approved_by,idempotency_key) SELECT r.system_id,r.rule_id,r.rule_version+1,r.rule_name,r.rule_definition,r.risk_level,#{mode},#{threshold},r.enabled,CURRENT_TIMESTAMP,#{actorId},#{reason},#{approverId},#{idempotencyKey} FROM security_rule r WHERE r.system_id=#{scope} AND r.rule_id=#{id} AND r.rule_version=#{version} AND NOT EXISTS (SELECT 1 FROM security_rule newer WHERE newer.system_id=r.system_id AND newer.rule_id=r.rule_id AND newer.rule_version>r.rule_version)")
    ...
    @Select("SELECT rule_id AS id,rule_mode AS mode,rule_version AS version,rule_threshold AS threshold,created_by AS actorId,approved_by AS approverId,change_reason AS reason,idempotency_key AS idempotencyKey FROM security_rule WHERE system_id=#{scope} AND rule_id=#{id} AND idempotency_key=#{idempotencyKey}")
