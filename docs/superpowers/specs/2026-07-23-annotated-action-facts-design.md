...
接入指南的矩阵必须把“组件支持”与“宿主必须接入”分开标识。这里的“必须”表示：只要宿主具备相应业务能力，就必须从该能力的服务端权威位置产生事件；它不是 Starter 的启动前置条件，也不会因为一个宿主不存在导出或角色管理功能而导致启动失败。

下列能力属于条件性接入基线，矩阵标记为“宿主必接”：

- 认证：`auth:login-success`、`auth:login-failure` 与适用时的 `auth:mfa-failure`；
- 会话：支持并发会话管理时的 `session:concurrent`；
- 授权：所有资源范围拒绝必须经 `ResourceAccessGuard` 记录为 `ACCESS_DENIED` 或 `RESOURCE_SCOPE_DENIED`；
- 数据：敏感查询/浏览、导出与批量数据操作；
- 权限：角色授予、管理员创建和其他实际提升权限的操作；
- 安全运维：规则、审计配置和安全开关变更。

普通查询、创建、更新、删除、登出、会话创建/撤销等不直接满足内置规则条件的动作标记为“可选增强”。它们仍可使用同一动作编码和采集机制，但不会被表述为宿主必须接入。每一行矩阵还要列出缺少必填动态事实时的降级结果，例如登录失败缺少 `attempted_account_hash` 时只能按来源 IP 回退聚合。

控制动作另以“触发方式”和“审核要求”展示，且必须说明 `OBSERVE` 从不执行宿主控制，以下自动行为仅在 `ENFORCE` 且宿主已注册匹配 `ControlHandler` 时发生：

| 控制动作 | 宿主接入要求 | 触发与审核策略 |
| --- | --- | --- |
| `RECORD` | 无 `ControlHandler` 要求 | 自动保留事件和告警证据，不改变业务请求，无需人工审核。 |
| `REQUIRE_CAPTCHA`、`RATE_LIMIT`、`REVOKE_SESSION`、`REQUIRE_MFA`、`DENY` | 宿主必须实现对应、幂等的 `ControlHandler` | 内置规则命中后自动执行，无需逐次人工审核；仍受宿主既有认证、授权和审计约束。 |
| `REQUIRE_APPROVAL` | 宿主必须实现审批工作流 `ControlHandler` | 自动创建或进入待审批状态，但不得自动放行；由被授权人员完成审批。 |
| `LOCK` | 仅宿主自定义规则可请求，且必须实现对应 `ControlHandler` | 内置规则不自动触发锁定；默认要求人工审核。宿主若要自动锁定，必须另行定义并评审自定义规则、解锁策略和误报处置。 |

矩阵中的每个内置规则行还要引用上述动作策略。例如 `AUTH-01`、`AUTH-02`、`AUTHZ-01` 和 `DATA-01` 的限流为自动触发；`DATA-02`、`EXPT-01` 和 `PRIV-02` 的 `REQUIRE_APPROVAL` 必须人工审批；`SECU-01` 只有 `RECORD`，不触发宿主业务控制。
