...
| 表 | 所有权与约束 |
| --- | --- |
| `security_event` | 事件根；按系统和时间建立索引。 |
| `security_event_fact` | 强类型事实值及来源；以事件和事实类型关联。 |
| `security_event_role`、`security_event_attribute`、`security_event_input_issue` | 事件的角色、受控扩展属性和输入问题。 |
| `security_rule` | 可查询的规则版本和启停状态；变更通过管理服务审计。 |
| `security_alert`、`alert_event_link`、`alert_disposition` | 告警、命中事件和追加式处置历史。 |
| `control_action`、`control_action_attempt` | 持久控制及每次执行尝试；幂等键防止重复执行。 |
| `security_whitelist` | 有时限的规则抑制，具有稳定业务标识。 |
| `notification_delivery` | 通知交付状态。 |
| `management_audit` | 管理查询、变更和拒绝记录，按系统与时间检索。 |
