...
角色职责和文档变更流程见[组织角色与文档管理](组织角色与文档管理.md)。本文只保留运行时、事务和生产值守约束。
...
```mermaid
flowchart LR
    Host["宿主应用"] --> Starter["spring2-starter / spring3-starter"]
    Starter --> API["api：模型、SPI、错误契约"]
    Starter --> Core["core：领域与应用服务"]
    Core --> API
    Core --> Port["core.port：仓储、控制、通知、事务"]
    Starter --> MyBatis["mybatis：仓储、Mapper、PO"]
    MyBatis --> Port
    MyBatis --> DB["监测审计数据库"]
    Port --> Control["宿主 ControlHandler"]
    Port --> Notify["宿主 NotificationChannel"]
```

依赖始终指向框架无关内层。`core.domain` 与 `core.domain.rule` 不依赖 Spring、Servlet、MyBatis 或宿主授权框架；用例编排在 `core.application`，可替换边界在 `core.port`。`mybatis.po` 是适配器私有的表行映射，不能进入 `api` 或替代不可变领域对象；`MyBatisMonitoringRepository` 是领域对象与 PO 的唯一转换位置。
...
sequenceDiagram
    ...
    participant External as 控制与通知

    Host->>Monitor: record(SecurityEventDraft)
    Monitor->>Repository: inTransaction(...)
    Repository-->>Monitor: 提交事件、匹配、告警与关联
    Monitor->>External: 通知并执行控制
    External-->>Monitor: 返回执行结果
```

一次 `inTransaction(...)` 包含事件写入、依赖历史的规则判定、告警摘要和事件关联；告警生命周期事务同时提交状态与仅追加处置。通知和宿主控制只在提交成功后执行，避免不可回滚的外部副作用先于审计事实发生。该事务不加入宿主业务事务；需要跨资源原子性时应使用宿主批准的事务发件箱或适配器。
