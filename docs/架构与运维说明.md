...
每次尝试追加到 `monitoring_control_action_attempt`。幂等键具有唯一约束；并发领取或状态迁移只能有一个版本成功。

管理写操作通过 `MonitoringTransaction` 组合业务状态和 `monitoring_management_audit`。授权拒绝也必须留下独立 `DENIED` 记录。管理审计是追加写，不提供更新或删除 Mapper。
...
| 聚合 | 表 |
| --- | --- |
| 事件 | `monitoring_security_event` 及 role/attribute/input_issue 子表 |
| 告警 | `monitoring_security_alert`、`monitoring_alert_event_link`、`monitoring_alert_disposition` |
| 控制 | `monitoring_control_action`、`monitoring_control_action_attempt` |
| 白名单/规则 | `monitoring_security_whitelist`、规则版本表 |
| 管理审计 | `monitoring_management_audit` |
