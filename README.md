...
面向 Spring Boot 2.1/2.7 和 Spring Boot 3 宿主的强类型异常访问监测组件。宿主保留认证、资源授权、会话和实际控制权；组件负责把可信服务端事实转换为事件，执行 14 条内置规则，持久化告警与控制，并提供可直接注入 Controller 的管理服务。
...
| `spring2-legacy-starter` / `spring2-starter` / `spring3-starter` | Boot 2.1 / 2.7 / 3 自动配置 |
...
通过 BOM 管理版本，并只引入一个与宿主 Boot 主版本匹配的 Starter（Boot 2.1 用 `spring2-legacy-starter`，Boot 2.7 用 `spring2-starter`）：
...
示例中的 Shiro Header 身份、固定报表和控制状态表仅用于验收，不应复制为生产认证、授权、资源计数或控制实现。
