...
A strict, typed monitoring component for Spring Boot 2.2/2.7 and Boot 3 hosts. The host remains authoritative for identity, resource authorization, sessions, endpoints, and real control effects. This component validates action facts, evaluates 14 built-in rules, persists alerts and durable controls through MyBatis, and exposes controller-ready management services.
...
| `spring2-legacy-starter` / `spring2-starter` / `spring3-starter` | Boot 2.2 / 2.7 / 3 auto-configuration |
...
1. Import the BOM and one starter matching the host Boot version (`spring2-legacy-starter` for Boot 2.2, `spring2-starter` for Boot 2.7, `spring3-starter` for Boot 3).
