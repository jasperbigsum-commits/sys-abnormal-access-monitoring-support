...
这是一个可运行的 Spring Boot 3.2.12 / `jakarta.servlet` 宿主夹具。它与 `spring2-web` 覆盖同一组验收编号，用于证明异常访问监测组件能够在 Boot 3、真实 HTTP、MyBatis 持久化、Shiro 身份、资源授权、宿主控制动作和管理审计之间形成闭环。
...
| 项目 | 当前夹具配置 | 生产宿主应做什么 |
| --- | --- | --- |
| Spring Boot | 3.2.12 | 保持使用 `sys-abnormal-access-monitoring-spring3-starter` |
| Servlet API | `jakarta.servlet` | Boot 3 项目不得再引入 `javax.servlet.*` |
| JDK | Java 17 | 使用 Java 17 或更高版本 |
| Shiro | 2.0.4，`jakarta` classifier | 统一使用 Jakarta 变体，避免混入未分类或 `javax` 依赖 |
...
spring:
  datasource:
    url: jdbc:h2:mem:audit-spring3;MODE=MySQL;DB_CLOSE_DELAY=0
    username: sa
    password:
...
`Spring3AuditApplication` 启动时依次运行组件的 `db/monitoring-schema.sql` 和夹具的 `db/audit-fixture-schema.sql`，并构造 MyBatis `SqlSessionFactory`。此做法仅便于演示；生产必须把监测 Schema 纳入 Flyway、Liquibase 或既有变更流程，不应在应用启动时重复执行 DDL。
...
通过 BOM 管理版本，只引入 Boot 3 Starter：
