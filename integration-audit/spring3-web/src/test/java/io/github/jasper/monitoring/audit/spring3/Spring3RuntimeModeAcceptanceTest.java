...
class Spring3RuntimeModeAcceptanceTest {
    ...
    void tc17_observeRecordsWithoutBlockingWhileEnforceAppliesControls() {
        try (ConfigurableApplicationContext observe = start("OBSERVE", "audit-spring3-tc17-observe")) {
        ...
        try (ConfigurableApplicationContext enforce = start("ENFORCE", "audit-spring3-tc17-enforce")) {
    ...
    private static ConfigurableApplicationContext start(String mode, String database) {
        return new SpringApplicationBuilder(Spring3AuditApplication.class)
